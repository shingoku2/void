...
class ProtocolBuffer {
	...
	public append(data: string | Buffer): void {
		...
		if (this.buffer.length - this.index >= toAppend.length) {
			...
			if (this.index === 0) {
				...
				this.buffer.set(toAppend, 0);
			} else {
				this.buffer = Buffer.concat([new Uint8Array(this.buffer.slice(0, this.index)), new Uint8Array(toAppend)], newSize);
			}
		}
	...
	public tryReadContent(length: number): string | null {
		...
		this.buffer.copyWithin(0, sourceStart, this.index);
